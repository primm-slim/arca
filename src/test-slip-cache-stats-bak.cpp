...
void test_simple_block() {
    ...
    START_TEST("Simple Block Test");

    blk.init(16);
    TEST_VAL(blk.get_block_ct(), 16);
    TEST_VAL(blk.get_row_ct(), 16);
...
/*
    for (Index i = 0; i != 7; i += 1) {
        ...
        Index h = hash_char_array(key_str);
        Index k = h & 15;
        if (blk.find_free_pos(k, h, pos)==0) {
            TEST_VAL(1, 0);
        } else {
...
void cache_test() {
    ...
    using Cache = SlipCache<SimpleBlockFn>;
